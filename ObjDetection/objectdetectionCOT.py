...
parser = argparse.ArgumentParser(description="Esegui test TAM e CoT su un sottoinsieme di immagini COCO.")
parser.add_argument(
    '-n', '--num_images', 
    type=int, 
    default=None, 
...
# Cartella dedicata per le TAM
tam_dir = os.path.join(output_dir, 'tam_heatmaps')
os.makedirs(tam_dir, exist_ok=True)

results_file = os.path.join(output_dir, 'results_phase4_cot.csv') 
...
# Liste per le medie degli IoU su tutte le attivazioni
lista_obj_iou = []
lista_func_iou = []
lista_f1_iou = []
lista_pixel_acc = []
...
for index, row in df.iterrows():
    ...
    prompt = f"Is there a {obj_main} in this image? Let's think step by step before answering."
    ...
    with torch.no_grad():
        outputs = model.generate(**inputs, max_new_tokens=150, use_cache=True, output_hidden_states=True, return_dict_in_generate=True)
    ...
    for i, t_id in enumerate(token_ids_generati):
        ...
        if parola_pulita in ["yes", "no"] or (obj_main.lower() in parola_pulita and len(parola_pulita) > 1):
            target_indices.append(i)
            target_token_texts.append(parola_token)
            
    if not target_indices:
        print("   [!] Nessun token chiave trovato nel ragionamento. Uso l'ultima parola come fallback.")
    ...
    for t_idx, t_text in zip(target_indices, target_token_texts):
        # Generiamo il nome del file della TAM (es. 00000438_step45.jpg)
        ...
        # Salviamo i risultati per la media finale
        ...
        is_correct_id = 0
        parola_pulita_check = "".join(c for c in t_text.lower() if c.isalpha())
        
        # Valutazione per SINGOLA ATTIVAZIONE
        if "no" in parola_pulita_check and "yes" not in parola_pulita_check:
            FN += 1
            print(f"      - Mappa '{t_text}' (Indice {t_idx}): FALSO NEGATIVO (Risposta errata)")
        else:
    ...
    del inputs, outputs, logits, generated_ids
    gc.collect()
...
print("\n" + "="*50)
print("🎯 REPORT METRICHE GLOBALI CoT (PER ATTIVAZIONE)")
print("="*50)
...
nome_prompt = "CoT_StepByStep_PerActivation"
