...
def build_token_groups(step_rows: list[dict]) -> list[dict]:
    ...
    def flush_current() -> None:
        ...
        groups.append({
            ...
            "source_folders": [row["folder_name"] for row in current],
        })
        current = []
    ...
    for group_idx, group in enumerate(token_groups):
        ...
        for layer_dir in sorted(image_vis_root.iterdir()):
            if not layer_dir.is_dir() or not layer_dir.name.startswith("layer_"):
                continue
            step_paths = []
            for step_idx, folder_name in zip(group["step_indices"], group["source_folders"]):
                preferred = layer_dir / f"{folder_name}.jpg"
                if preferred.exists():
                    step_paths.append(preferred)
                    continue
                step_paths.append(layer_dir / f"step_{step_idx:04d}.jpg")
