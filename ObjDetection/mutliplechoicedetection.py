...
parser = argparse.ArgumentParser(description="Esegui test TAM e MCQ su un sottoinsieme di immagini COCO.")
parser.add_argument(
    '-n', '--num_images', 
    type=int, 
    default=None, 
...
results_file = os.path.join(output_dir, 'results_mcq.csv') 

print("Inizializzo COCO e recupero le categorie...")
coco = COCO(ann_file)
df = pd.read_csv(csv_path)

# Estrazione delle categorie per i distrattori
...
for index, row in df.iterrows():
    ...
    prompt = f"Choose which of these objects is in the image: {opzioni_formattate}."
    system_prompt = "You are an expert computer vision model trained to accurately identify objects in images. Only answer with the correct choice."
    ...
    if not ha_risposto_correttamente:
        print(f"   [!] Il modello ha scelto un distrattore o non ha risposto correttamente (FN).")
        FN += 1
        risultati.append({
    ...
    # --- LOGICA INTELLIGENTE DI SELEZIONE TOKEN (Parola vs Lettera) ---
    # Se l'oggetto NON è nel testo, significa che il modello ha risposto SOLO con la lettera
    ...
    for i, t_id in enumerate(token_ids_generati):
        ...
        if cerca_solo_lettera:
            # Stiamo cercando il token contenente esclusivamente la lettera
            if parola_pulita == lettera_corretta.lower():
                target_idx = i
                target_token_text = parola_token
                break
        else:
            # Il modello ha scritto la parola intera, quindi cerchiamo il token dell'oggetto
    ...
    print(f"   -> Estraggo mappa TAM basandomi sul token: '{target_token_text}'")
    ...
    if obj_iou >= SOGLIA_IOU_CORRETTO:
        TP += 1
        is_correct_id = 1
        print(f"   [+] Identificazione Corretta e Centrata! (Obj-IoU={obj_iou:.3f} >= {SOGLIA_IOU_CORRETTO})")
    else:
        FP += 1
        print(f"   [-] Ha indovinato, ma localizzazione Errata. (Obj-IoU={obj_iou:.3f} < {SOGLIA_IOU_CORRETTO})")

    print(f"   -> Metriche calcolate sul token '{target_token_text}': Obj-IoU={obj_iou:.3f}, Pixel-Acc={pixel_acc:.3f}")
    ...
    del inputs, image_inputs, video_inputs
    del outputs, logits, generated_ids, heatmap
...
print("\n" + "="*50)
print("🎯 REPORT METRICHE GLOBALI DEL DATASET (MCQ)")
print("="*50)
