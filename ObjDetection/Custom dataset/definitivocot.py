...
csv_path = './dataset_custom.csv' # AGGIORNATO
ann_file = './instances_Test.json' # AGGIORNATO
output_dir = './output/test_cot_custom_dataset/' # Cartella per i nuovi risultati
vis_dir = os.path.join(output_dir, 'visualizations')
os.makedirs(vis_dir, exist_ok=True)
results_file = os.path.join(output_dir, 'results_cot_custom.csv')
...
for index, row in df.iterrows():
    ...
    messages = [
        ...
        {"role": "user", "content": [
            {"type": "image", "image": img_path, "max_pixels": 313600}, 
    ...
    with torch.no_grad():
        outputs = model.generate(**inputs, max_new_tokens=150, do_sample=False, use_cache=True, output_hidden_states=True, return_dict_in_generate=True)        
    ...
    # ---------------------------------------------------------
    # NUOVO PARSER SEMANTICO AVANZATO (REGEX) - CORRETTO
    # ---------------------------------------------------------
    target_clean = re.escape(target_obj.lower())
    
    # FIX: Usiamo le doppie parentesi graffe {{0,40}} affinché la f-string di Python 
    # le passi correttamente alla Regex come quantificatore.
    # Aggiunto re.DOTALL per far funzionare il '.' anche se ci sono andate a capo (\n).
    pattern_negazione = re.compile(rf"\b(no|not|without|cannot|n't)\b.{{0,40}}\b{target_clean}(?:s|es)?\b", re.DOTALL)
    
    # Pattern 2: Verifica la presenza grezza della parola target
    menziona_target = re.search(rf"\b{target_clean}(?:s|es)?\b", testo_pulito) is not None
