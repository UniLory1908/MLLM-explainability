...
parser = argparse.ArgumentParser(description="VQA Misto (75-25) con Centroide Pesato e Fallback su Massimo.")
...
output_dir = './output/test_mixed_hard_centroid/'
tam_dir = os.path.join(output_dir, 'tam_heatmaps')
os.makedirs(tam_dir, exist_ok=True)
results_file = os.path.join(output_dir, 'results_mixed_hard_centroid.csv') 
...
for index, row in df.iterrows():
    ...
    if query_type == "Positive":
        if ha_negato or not ha_confermato:
            ...
            # 1. Carica la Maschera COCO per ottenere le dimensioni originali
            maschera_coco = coco.annToMask(coco.loadAnns(ann_id_main)[0]) 
            orig_h, orig_w = maschera_coco.shape  # Dimensioni reali dell'immagine (es. 480x640)
            
            vision_shape = (inputs['image_grid_thw'][0, 1] // 2, inputs['image_grid_thw'][0, 2] // 2)
            percorso_tam = os.path.join(tam_dir, f"{row['path']}_pos_centroid.jpg")
            ...
            # --- FIX: Ridimensionamento della TAM alle dimensioni della Ground Truth ---
            heatmap_norm = cv2.resize(heatmap_norm, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
            
            # GATE 2: Coerenza Strutturale (Scattering Check)
            ...
            if num_hotspots > MAX_HOTSPOTS:
                ...
                if num_hotspots == 0:
                    print(f"   [⚠️] Nessun hotspot sopra soglia rilevato. Attivo Fallback sul Massimo Assoluto...")
                    cX, cY = max_x, max_y
                    used_fallback = True
                else:
                ...
                if maschera_coco[cY, cX] == 1:
                    ...
                    if num_hotspots == 0:
                        print(f"   [->] Puntamento tramite Massimo Assoluto ({cX}, {cY}) DENTRO la Ground Truth.")
                    else:
                        print(f"   [->] Centroide Localizzato ({cX}, {cY}) DENTRO la Ground Truth.")
                else:
                    if num_hotspots > 0:
                ...
                if not pointing_success:
                    ...
                    # GATE 4: Confidenza Visiva (EIG)
                    # Ora maschera_coco e heatmap_norm combaciano perfettamente!
                    ...
                    if eig < SOGLIA_EIG:
                        print(f"   [!] FP: Energia interna insufficiente (EIG: {eig:.2f} < {SOGLIA_EIG}). Fallito Gate 4.")
                        ...
                        print(f"   [+] TP Convalidato! (EIG: {eig:.2f}, Hotspots: {num_hotspots}, Fallback: {used_fallback})")
        ...
        if ha_negato or not ha_confermato:
            ...
            # Leggiamo le dimensioni originali dell'immagine per fare l'upscaling dell'allucinazione
            orig_img = cv2.imread(img_path)
            if orig_img is not None:
                orig_h, orig_w = orig_img.shape[:2]
            else:
                orig_h, orig_w = 480, 640 # Safety fallback
            ...
            # --- FIX: Ridimensionamento della TAM anche per le negative queries ---
            heatmap_norm = cv2.resize(heatmap_norm, (orig_w, orig_h), interpolation=cv2.INTER_LINEAR)
...
print("\n" + "="*60)
print("🎯 REPORT ESPERIMENTO: CENTROID-BASED + CO-FALLBACK GAME")
print("="*60)
...
print(f"   └─ Salvati da Fallback (Max Peak / 0 Hotspots): {Pos_TP_Fallback}")
...
print(f"   └─ Bassa Concentrazione Energetica (EIG < {SOGLIA_EIG}): {Pos_FP_LowEnergy}")
